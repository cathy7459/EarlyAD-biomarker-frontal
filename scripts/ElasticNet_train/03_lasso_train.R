...
# ------------------------------------------------------------
meta_path_candidates <- c(
  file.path("data", "processed", "sample_metadata_gse33000.csv"),
  file.path("data", "processed", "GSE33000_pheno.csv")
)

meta_path <- meta_path_candidates[file.exists(meta_path_candidates)][1]

if (is.na(meta_path) || !nzchar(meta_path)) {
  stop(
    "Missing metadata file.\nChecked:\n",
    paste(meta_path_candidates, collapse = "\n")
  )
}

log_msg("[2/4] Loading metadata: ", meta_path)

meta_raw <- as.data.frame(fread(meta_path))

# Case 1: already standardized metadata
if (all(c("sample_id", "condition") %in% names(meta_raw))) {
  meta <- meta_raw[, c("sample_id", "condition"), drop = FALSE]
  
} else {
  # Case 2: raw pheno table like GSE33000_pheno.csv
  if (!("Sample" %in% names(meta_raw))) {
    stop(
      "Metadata must contain either columns 'sample_id' and 'condition', ",
      "or a raw pheno column named 'Sample'."
    )
  }
  
  log_msg("Metadata does not have standardized columns -> deriving sample_id / condition from raw pheno table")
  
  pheno_text <- apply(meta_raw, 1, function(x) paste(as.character(x), collapse = " | "))
  
  condition <- ifelse(
    grepl("Alzheimer's disease", pheno_text, ignore.case = TRUE), "AD",
    ifelse(grepl("non-demented", pheno_text, ignore.case = TRUE), "Control", NA)
  )
  
  meta <- data.frame(
    sample_id = trimws(as.character(meta_raw$Sample)),
    condition = condition,
...
if (nrow(meta) == 0) {
  stop("No AD / Control samples found in metadata after parsing.")
}
