...
# ------------------------------------------------------------
# Resolve core gene file path robustly
# ------------------------------------------------------------
core_gene_path <- NULL

# 1) first priority: CFG$core_gene_csv
...
# 2) fallback candidates relative to project
...
if (!is.null(core_gene_path) && file.exists(core_gene_path)) {
  ...
  if (exists("CFG")) CFG$core_gene_csv <- core_gene_path
  ...
  log_msg("[Core] searched CFG$core_gene_csv and default relative paths under data/processed and outputs/")
}
